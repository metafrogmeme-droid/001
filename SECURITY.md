...
- **Bearer Token Authentication** — the API bridge's `/analyze`, `/portfolio` and `/risk/status` require a `DASHBOARD_TOKEN` bearer token; `/health` and `/scan` do not. `/confirm`, `/portfolio/close` and `/risk/halt` are token-gated and **refuse**: the bridge is a separate process from the trading bot, its engine is a read-only copy of the bot's state, and nothing it did there reached the bot. Halt the bot from Telegram with `/halt` or `/emergency_stop`.
