...
### 十四種語言介面 (NEW)
整個 Telegram 介面皆**以與網站相同的十四種語言在地化** —— English、Español、繁體中文、Português、Français、Deutsch、Nederlands、日本語、한국어、Русский、Türkçe、Italiano、हिन्दी 與 العربية：
- **每位使用者的語言偏好** —— 每位使用者以 `/lang` 選擇自己的語言（鍵盤，或 `/lang fr`、`/lang français`、`/lang french`）；新使用者的 Telegram 客戶端語言會自動作為初始值。該選擇會逐一使用者儲存，因此訊息、`/help`、「/」指令選單、警示與風險說明都會以所選語言呈現。
- **完整涵蓋** —— 每個鍵在每種語言都存在：英文與繁體中文內嵌於 `bot/utils/i18n.py`，其餘十二種語言各一個檔案放在 `bot/utils/locales/`，`/help` 與指令選單則在 `bot/skills/command_catalog_locales/`。`tests/test_i18n_locales.py` 鎖定機器人的語言清單與網站一致、每個鍵在每種語言都有、以及佔位符／HTML 與英文對齊。
- **安全後備** —— 任何未翻譯的鍵都會回退為英文，而非失敗；模型能回答但字典未涵蓋的語言，會在在地化的回答外顯示英文介面文字。
