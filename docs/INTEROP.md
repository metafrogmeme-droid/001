...
| Machine-consumable tools | `POST /mcp` (31 read-only tools + 3 key-gated Arena tools), discoverable at `/.well-known/mcp.json`, documented at `/developers` | Any MCP-capable agent consumes the above without trusting this server: the re-derivation recipe rides the payload |
