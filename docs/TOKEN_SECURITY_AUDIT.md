...
- ~~Sale economics agree field by field across `smithii.config.json`, `metaplex-genesis.config.json` and the roadmap~~ **CORRECTED 2026-07-27 — this was wrong, and the way it was wrong is worth keeping visible.** The fields it went on to enumerate do all agree: 150,000,000 presale allocation, 1000/5000 SOL soft and hard caps, 0.25/25 SOL contribution bounds, a 48h whitelist followed by a 72h public round that the absolute timeline at `metaplex-genesis.config.json:33-35` reproduces exactly, and 33% at TGE encoded as `cliffAmountBps: 3300` with a two-month linear tail. But "agree field by field" is a claim about the *whole* key set, and the enumeration stops one field short of three that did not agree: `refundIfSoftCapMissed` (`true` at Smithii against `false` at Genesis), `autoListPercentOfRaise` (60 against 6667 bps = 66.67%), and the LP lock (12 months against never-claim/permanent). The refund divergence is the one that mattered: `metaplex-genesis.config.json` documents that flag as a promise which must not ship — `derivePresaleParams()` *throws* if it is set — so the primary venue could not publish it even by accident, while the fallback published it by default. A list of things that match cannot establish that nothing mismatches. `token/presale/venue_parity.test.mjs` now compares the full key set, so the parity claim is machine-checked rather than enumerated; the three values are corrected in `smithii.config.json` with the reasoning inline.
