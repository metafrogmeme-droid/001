...
function writeDerivedConfig() {
  ...
  // finalizeV2 refuses unless the WHOLE supply is allocated across buckets:
  //   Program log: Total supply must be fully allocated before finalize
  // The committed config declares 1,000,000,000 total while this harness only
  // creates two buckets (150M presale + 100M liquidity = 250M), so a real launch
  // needs buckets for the remaining 75% — team, treasury, community, the rest of
  // the §4 allocation table — and that tooling does not exist yet.
  //
  // For the dry run the supply is narrowed to exactly what these two buckets
  // hold, which makes the lifecycle exercisable without inventing allocations
  // nobody has ratified. This is a HARNESS simplification and is called out in
  // the report it writes: it does not make the committed config finalizable.
  const allocatedSupply = (
    BigInt(base.sale.presaleAllocation) + BigInt(base.liquidity.tokenAllocation)
  ).toString();
  const cfg = {
    ...base,
    _comment: 'GENERATED near-now dry-run config — do not commit. See token/e2e/.',
    token: { ...base.token, totalSupply: allocatedSupply },
...
function saveReport(steps) {
  const report = {
    ...
    harnessSimplifications: [
      'token.totalSupply narrowed to presaleAllocation + liquidity.tokenAllocation so ' +
      'finalizeV2 can run; the committed 1B config is NOT finalizable until buckets exist ' +
      'for the remaining ~75% of supply.',
    ],
