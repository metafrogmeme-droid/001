    ...
    return [
        ...
        ("Judgment calls (operator decision)", [
            ...
            ("USER_RISK_PREF_SIZING_ENABLED",
             "Apply a user's declared risk appetite to their size (tighten-only)",
             bool(_attr("risk", "user_risk_pref_sizing_enabled"))),
            ("QUALITY_LADDER_SIZE_ENABLED",
             "Trade quality (measured confidence) shrinks size by rung (tighten-only)",
             bool(_attr("risk", "quality_ladder_size_enabled"))),
            ("QUALITY_LADDER_LEVERAGE_ENABLED",
             "Trade quality caps the leverage the venue is set to, by rung (reduce-only)",
             bool(_attr("risk", "quality_ladder_leverage_enabled"))),
