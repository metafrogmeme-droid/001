...
def send_outcome_unknown(exc: BaseException) -> bool:
    """Whether a raise from the close order's SEND leaves it unknown if the
    venue received the order.

    ccxt files everything transport-shaped under ``NetworkError``, and much
    of that family is the venue ANSWERING no — a rate limit (429, 418,
    Bitget's 1001/50009), maintenance, a bad nonce. Those are refusals: the
    order never reached the book, and reading them as "may have reached the
    venue" told every post-fill guard to stand down on a position whose
    ...
    which ccxt files under the generic ``ExchangeError``); and a **5xx**,
    which says the request reached a server that then failed — whether the
    order reached the book is exactly what nobody knows. A first draft called
    the 5xx a refusal, which is a verdict from no reading.
    ...
    try:
        import aiohttp
    except ImportError:                      # pragma: no cover — ccxt needs it
        return False
    cause = exc.__cause__
    if isinstance(cause, aiohttp.ClientPayloadError):
        return True                          # answered, then the body was cut
    if not isinstance(exc, ccxt.ExchangeNotAvailable):
        return False
    if isinstance(cause, aiohttp.ClientConnectorError):
        return False                         # never connected: nothing was sent
    if isinstance(cause, aiohttp.ClientConnectionError):
        return True                          # dropped after the send
    # No aiohttp cause: an HTTP 5xx, or a venue error code ccxt maps here.
    # The two are indistinguishable from the exception, and the honest
    # reading of the pair is the unconfirmed one.
    return cause is None


def stop_replacement_note(stop_loss: Any, take_profit: Any,
                          direction: Any = None) -> str:
    """What the monitor will do about a position left without a stop.

    The periodic re-place (and the grace one) runs only for a pair
    ``_place_sl_tp`` will accept, and it refuses on BOTH clauses of
    ``_sltp_side_error``: a non-positive leg, and a wrong-sided pair. Every
    card used to promise "the monitor re-places one on its next pass"
    unconditionally, which is false for an adopted or emergency position with
    no target — those are price-monitored only, and the operator has to put
    the stop on by hand.

    The negative branch names WHAT is missing rather than assuming it is the
    target: a first draft said "it carries no target level" for a position
    whose target was the leg it had. ``direction`` is optional because the
    entry guards call this before the pair reaches a position; without it the
    side clause cannot be checked and only the levels are read.
    """
    try:
        ...
        sl = tp = 0.0
    if sl <= 0 or tp <= 0:
        missing = ("no stop level and no target" if sl <= 0 and tp <= 0
                   else "no target level" if tp <= 0 else "no stop level")
        return (f"it carries {missing}, so the monitor will NOT re-place a "
                f"stop on it — put one on it by hand")
    if direction is not None:
        is_long = direction == Direction.LONG or str(direction).upper().endswith("LONG")
        if (sl < tp) is not is_long:
            return ("its stop and target are on the wrong sides of each other, "
                    "which the venue-side placement refuses, so the monitor will "
                    "NOT re-place a stop on it — put one on it by hand")
    return ("if it is still there without a stop the monitor re-places "
...
def close_pnl_line(net_pnl, pnl_pct, pnl_pct_margin, leverage, commission):
    ...
    instead of grepped for. The three readings fail INDEPENDENTLY: the venue
    can report a realized PnL for a close whose exit price no source would
    give up (money known, percentages not), and it can report neither.

    ``unread`` wherever a number cannot be had -- never 0, and never a
...
def close_pct(exit_price, entry_price, direction, leverage):
    """(notional %, margin %) for a close, or (None, None) when unpriceable.

    An unread exit and a zero entry are both absences; neither is a return.
    """
...
class LiveExecutor:
            ...
            if _outcome == "kept_open":
                audit(trade_log,
                      ...
                      data={"trade_id": trade_id, "symbol": pos.symbol,
                            "requested": want, "actual": got, "path": context,
                            "close_msg": close_msg})
                _re_place = stop_replacement_note(
                    getattr(pos, "stop_loss", None), getattr(pos, "take_profit", None),
                    getattr(pos, "direction", None))
                return (
                    ...
                    f"line below says what it kept, and {_re_place}. "
    ...
    async def _post_fill_slippage_guard(self, idea: TradeIdea, fill_price: float,
        ...
        complete and close_position kept it tracked (its answer says whether
        a stop was re-placed), so nothing more may be placed on it. ``warn``
        is the URGENT note for the
    ...
    async def _leverage_overshoot_guard(self, idea: TradeIdea, symbol: str,
        ...
        and kept it tracked — its answer says whether a stop was re-placed —
        so nothing more may be placed on it; else None. ``_lev_close_failed``
        is True when the flatten did not
            ...
            if _lev_mismatch is not None:
                ...
                if _lev_verdict["decision"] == "close":
                    ...
                    if _outcome == "failed":
                        ...
                        #
                        # And in its OWN handler, not the close's. The rest
                        # used to sit inside the try above, so a failure
                        # here — AFTER a successful close — was caught as
                        # "flatten FAILED": it audited CLOSE_FAILED, told
                        # the card the position was OPEN, and fell through
                        # to place a stop and a take-profit against a
                        # position that no longer existed. That is the
                        # resurrection hazard `_lev_flattened` exists to
                        # prevent, reached through the one handler that
                        # runs before the flag is read. The position is
                        # closed either way; only the cooldown is in doubt,
                        ...
                        try:
                            ...
                            logger.error(
                                "Leverage-overshoot cooldown NOT set for %s after "
                        ...
                        if _outcome == "kept_open":
                            ...
                            # close_position kept it tracked, and its own
                            # line says whether a stop was re-placed (its
                            # final handler re-places nothing). Returning
                            # here is what keeps
                            ...
                            return (
                                ...
                                f"line below says what it kept, and "
                                f"{stop_replacement_note(idea.stop_loss, idea.take_profit, idea.direction)}. "
    ...
    async def _guard_unprotected_grace(self, exchange, pos) -> Optional[str]:
        ...
        for i in range(max_iter):
            ...
            if breached:
                ...
                # close_position answers by RETURN VALUE. This audit used to
                # say CLOSED_LOCAL before anyone read the answer, so a
                # rejected or kept-open breach-close entered the chain as a
                # close; the ladder reads the answer, and so does the chain.
                _outcome = flatten_outcome(msg)
                _result = {"closed": "CLOSED_LOCAL",
                           "failed": "CLOSE_FAILED"}.get(_outcome, "NOT_CLOSED")
                _what = {"closed": "closed",
                         "failed": "could NOT close (rejected)"}.get(
                    _outcome, "did NOT complete the close of")
                audit(trade_log,
                      f"Grace sub-loop {_what} UNPROTECTED {pos.symbol} on {reason} "
                      f"@ ${price:.6f}",
                      action="grace_guard", result=_result,
                      data={"trade_id": pos.trade_id, "symbol": pos.symbol,
                            "reason": reason, "price": price, "iterations": i + 1,
                            "close_msg": None if _outcome == "closed" else msg})
                return msg
    ...
    async def _reattempt_post_fill_sl(
        ...
        if sl_id is None and pos.stop_loss > 0:
            ...
            if _outcome == "kept_open":
                ...
                # whether a stop was re-placed, and its final handler
                # re-places nothing. Not a close, so not "CLOSED for safety" — the words
    ...
    # ── the protective leg is cancelled where it lives ───────────────────

    def _v3_strategy_cancel_sync(self, oid: str) -> dict:
        """POST /api/v3/trade/cancel-strategy-order for one strategy order.

        The v3 combined stop is a STRATEGY order (``_place_sl_tp_v3`` posts it
        to place-strategy-order, type ``tpsl``), and a strategy id handed to
        the regular cancel-order endpoint — where ccxt's ``cancel_order``
        routes a cancel with no trigger flag under UTA — answers 25204
        "Order does not exist" whether or not the stop is live. Read as
        "gone", that recorded a live stop as removed on every rejected close.
        Same transport as the placement, same contract: the venue's envelope
        comes back, an HTTP error's JSON body comes back as the envelope, and
        a request that never completed comes back as ``{"code":
        "TRANSPORT"}`` — distinct from a refusal, because a cancel nobody
        could send is not a cancel the venue declined. Synchronous; callers
        ...
        from bot.core.bitget_v3_client import BitgetV3Client
        try:
            return cast(dict, BitgetV3Client.for_account(self._credentials).request(
                "POST", "/api/v3/trade/cancel-strategy-order", {"orderId": str(oid)}))
        except Exception as e:
            if hasattr(e, "read"):
                try:
                    return cast(dict, _json.loads(e.read().decode()))
                except Exception:
                    return {"code": "TRANSPORT",
                            "msg": f"unreadable error body ({getattr(e, 'code', '?')})"}
            return {"code": "TRANSPORT", "msg": str(e)}

    def _v3_strategy_order_resting_sync(self, oid: str) -> Optional[bool]:
        """Is strategy order ``oid`` still among the account's unfilled tpsl
        orders? THREE-valued: True it is listed and not terminal, False the
        venue listed its unfilled strategy orders and this id is not among
        them, None the list could not be read. A cancel-strategy-order
        success "only indicates receipt" (docs/bitget-uta), so this is the
        reading that lets the record forget the id. Synchronous."""
        from bot.core.bitget_v3_client import BitgetV3Client
        try:
            data = BitgetV3Client.for_account(self._credentials).get(
                "/api/v3/trade/unfilled-strategy-orders?category=USDT-FUTURES&type=tpsl")
        except Exception as exc:
            logger.warning("v3 unfilled strategy orders read raised: %s", exc)
            return None
    ...
    async def _cancel_stop_leg(self, exchange, pos: LivePosition, oid: str,
        ...
        Returns ``(verdict, detail)``, verdict one of::

            removed     the venue cancelled it — a terminal cancel status, a
                        follow-up read, or the strategy list confirming it
            gone        the table that holds it says it does not exist: it
                        fired, or had already been cancelled
            live        the venue refused the cancel, or the order is still
                        resting after it
            unverified  the cancel was sent and nobody could confirm what
                        became of the order

        Only ``live`` keeps the id: the venue said the order is still there,
        so the record naming it is true. The other three clear it, including
        ``unverified`` — and that is the whole point of the third value.
        Nothing in this repo reads ``sl_order_id`` three-valued: non-empty is
        ...
        if combined and self._venue.id == "bitget":
            ...
            if code == "00000":
                resting = await asyncio.to_thread(self._v3_strategy_order_resting_sync, oid)
                if resting is False:
                    return "removed", "cancel accepted; the unfilled strategy list no longer carries it"
                if resting is True:
                    return "live", "cancel accepted, but the strategy order is still listed unfilled"
                return "unverified", "cancel accepted; the unfilled strategy list could not be read"
            # TRANSPORT first: its msg is `str(exc)` of an arbitrary transport
            # failure, and a substring test run before it would read a message
            # that happens to carry "does not exist" as the venue's answer.
            if code == "TRANSPORT":
                return "unverified", f"the cancel request did not complete ({msg[:120]})"
            if code == "25204" or "does not exist" in msg.lower():
        ...
        cancel_status = order_status(cancel_resp)
        if cancel_status in ("canceled", "cancelled", "closed"):
            return "removed", f"cancel answered {cancel_status}"
        # No status, or a non-terminal one: a cancel that returned is not a
        ...
        after = order_status(order_info)
        if after in ("canceled", "cancelled", "closed", "expired"):
            return "removed", f"follow-up read: {after}"
        return "live", f"still {after or 'unstated'} after the cancel"
    ...
    async def _close_position_inner(self, trade_id: str, reason: str = "bot_auto",
        ...
        try:
            ...
            # The combined id is the v3 strategy order, cancelled in the
            # strategy table by _cancel_stop_leg; remembered here so the
            # post-close sweep sends its re-cancel to the same table.
            _combined_ids = {oid for oid in _legs if oid == pos.sl_order_id == pos.tp_order_id}
            for oid in _legs:
                combined = oid in _combined_ids
                order_label = "SL/TP" if combined else ("SL" if oid == pos.sl_order_id else "TP")
                # Four verdicts, and only two of them let the record forget
                # the id. We still send the reduceOnly market close below on
                ...
                if _leg_verdict == "gone":
                    # "Fired, or already cancelled" — two readings, one
                    # outcome, so the result says what was read (GONE) rather
                    # than picking one of them (TRIGGERED). Nothing to
                    # re-cancel either, so this leg does not arm the
                    # post-close sweep, which cancels EVERY open order on the
                    # symbol and can take a resting entry limit with it.
                    audit(trade_log,
                          f"{order_label} order already gone from the venue: {pos.symbol} "
                          f"(order {oid}) — {_leg_detail}",
                          action="sltp_exchange_trigger", result="GONE",
                          data={"trade_id": trade_id, "order_id": oid,
                                "order_type": order_label, "symbol": pos.symbol})
                elif _leg_verdict == "live":
                    audit(trade_log,
                          f"Failed to cancel {order_label} order {oid} for {pos.symbol}: {_leg_detail}",
                          action="sltp_cancel_fail", result="ERROR",
                          data={"trade_id": trade_id, "order_id": oid,
                                "order_type": order_label, "symbol": pos.symbol,
                                "error": _leg_detail})
                    cancel_failed.append(oid)
                elif _leg_verdict == "unverified":
                    # Sent, not confirmed: the id stays on the record — an
                    # order nobody saw go may still be live, and a forgotten
                    # live stop the record does not name is not protection.
                    logger.warning("%s order %s for %s not verified gone: %s — id kept",
                                   order_label, oid, pos.symbol, _leg_detail)
                    audit(trade_log,
                          f"{order_label} order {oid} for {pos.symbol} could not be verified "
                          f"gone: {_leg_detail}",
                          action="sltp_cancel", result="UNVERIFIED",
                          data={"trade_id": trade_id, "order_id": oid,
                                "order_type": order_label, "symbol": pos.symbol})
                    cancel_failed.append(oid)
            ...
            # Use verified fill data when available. `_fill_src` records WHICH
            # read produced the exit price, so the persisted record can say
            # whether it is exchange truth or an inference -- and, at the end
            # of the chain, that nobody read it at all.
            _fill_src = "unread"
            if close_verify["fill_price"] > 0:
                fill_price = close_verify["fill_price"]
                _fill_src = "exchange_fill"
            else:
                # Fallback: extract from create_order response
                fill_price = float(order.get("average", 0) or order.get("price", 0) or 0)
                if fill_price > 0:
                    _fill_src = "close_order"
            ...
            # No absolute fallback to pos.entry_price here, deliberately.
            # Booking the exit AT the entry is an exact break-even that nobody
            # measured -- the calmest record there is, assembled from three
            # failed reads, on the close of a real position. `fill_price` stays
            # 0.0 meaning UNREAD; the venue's own PnL below may still price the
            # close, and if it does not the record carries None, which every
            # reader already scores as unscorable rather than as a flat trade
            # (bot/utils/win_rate.py, bot/backtest/parity.py both say so).
            ...
            # Recomputed HERE, not at the ticker fallback: the position-history
            # lookup above overwrites fill_price when its record carries a
            ...
            if exchange_pnl is not None:
                ...
                # lie -- but price none of it.
                gross_pnl = net_pnl = commission = None
                logger.warning(
                    "Close of %s booked UNPRICED: no venue PnL and no exit "
                    "price from any source (order, cost/filled, ticker).",
                    pos.symbol)
                audit(trade_log,
                      f"{pos.symbol} closed but no exit price could be read — "
                      f"booked UNPRICED (excluded from win rate and realized PnL)",
                      action="live_close", result="UNPRICED",
                      data={"trade_id": trade_id, "symbol": pos.symbol,
            ...
            pos.close_reason = reason
            pos.status = "closed"
            pos.close_price = fill_price if exit_price_known else None
            ...
            if cancel_failed:
                for stale_oid in cancel_failed:
                    try:
                        if stale_oid in _combined_ids and self._venue.id == "bitget":
                            # The strategy order lives in the strategy table;
            ...
            audit(trade_log,
                  f"Live position closed: {pos.symbol} net={money(net_pnl)} "
                  f"(gross={money(gross_pnl)}, fee={money(commission)})",
                  action="live_close",
            ...
            if not exit_price_known:
                close_msg += (
                    ...
                    "recorded UNPRICED — left out of win rate and realized PnL "
                    "rather than booked at the entry price, which would read as "
                    "a measured break-even. The venue's own history is the "
                    "place to price it."
                )

            # Store structured close data for rich rendering. `trade_id` is
            # what lets a reader that knows which close it is asking about
            # refuse an earlier close of the same symbol — the slot is
            # last-write-wins and the symbol alone cannot tell them apart.
            ...
            if "25227" in exc_str or "No position available" in exc_str:
                ...
                if still_open:
                    ...
                    try:
                        ...
                        # `all([])` is True: an accepted request that applied
                        # NOTHING must not read as a flat book either.
                        _flash_ok = (_flash_env.get("code") == "00000"
                                     and bool(_flash_items) and all(
            ...
            if pos.status == "closed" or trade_id not in self._positions:
                ...
                self._save_positions()
                # "The ledger row is written" is a reading, not a promise:
                # the append skips a cross-record duplicate and the save
                # swallows write failures. Reading the in-memory list caught
                # only the first of those — a disk-full write leaves the row
                # in memory and the sentence still claimed it. The row must
                # be in the list AND the last write must have landed.
                _row_written = (any(ct.trade_id == trade_id for ct in self._closed_trades)
                                and self._save_closed_trades())
                # No card was built for this close, so the shared close slot
                # still holds an EARLIER close — the renderers must not use it.
                self._last_close_data = None
                audit(trade_log,
                      ...
                      data={"trade_id": trade_id, "error": exc_str,
                            "close_order_id": close_order_id,
                            "ledger_row_written": _row_written})
                _ledger = ("The ledger row is written; check it if the numbers matter."
                           if _row_written else
                           "The ledger row could NOT be confirmed — the close is booked "
                           "on the record but may be missing from the ledger; check it.")
                return (
                    f"✅ CLOSED {pos.direction} {pos.symbol} — booked; "
                    f"{CLOSE_CARD_NOT_RENDERED} ({type(exc).__name__}). {_ledger}"
                )

            # H-01 FIX: Revert status so position is retried next cycle
            pos.status = "open"
            # The cancel pass ran before the close order, so a record that
            # still names a stop it removed claims a protection that is not
            # there. Only a leg the venue said is STILL LIVE keeps its id;
            # every other verdict — removed, gone, or nobody could tell —
            # clears it, because non-empty is "protected" to every reader and
            # an unverifiable stop must not answer ✅. The periodic stop check
            # re-places on empty ids, and its placement cancels what it finds
            # first, so clearing is the self-healing direction either way.
            if _cancel_pass_done:
                ...
                if pos.tp_order_id in _cancelled_ids:
                    pos.tp_order_id = None
            self._save_positions()
            # A timeout or a dropped connection while SENDING the close order
            # is not a rejection: the venue may have received and filled it.
            # Reading it as "CLOSE FAILED" would have the guards put a stop
            # on a book that is most likely flat; it is an unconfirmed close.
            # The rest of ccxt's NetworkError family — rate limits, 5xx,
            # maintenance, a bad nonce — is the venue ANSWERING no, and stays
            # a failed close (send_outcome_unknown says which is which).
            _close_unknown = (_cancel_pass_done and close_order_id is None
                              and send_outcome_unknown(exc))
            if close_order_id is not None or _venue_flat or _close_unknown:
                ...
                return (
                    ...
                    f"reconcile books it from the venue's own history, and "
                    f"{stop_replacement_note(pos.stop_loss, pos.take_profit, pos.direction)}. "
                    f"Review on Bitget."
                )
    ...
    def _append_closed_trade(self, pos: LivePosition) -> bool:
        """Append a closed trade to the persisted closed trades file.

        Returns whether the row is now IN the ledger on disk — False when the
        cross-record dedup skipped it, and False when the write failed. The
        booked-but-unreported answer tells the operator "the ledger row is
        written"; that sentence has to be a reading.
        ...
        return self._save_closed_trades()

    def _save_closed_trades(self) -> bool:
        """Persist all closed trades to disk. True when the write landed.

        It still swallows the failure — a ledger write must not take the
        close down — but it no longer swallows the FACT of it: a caller that
        tells the operator "the ledger row is written" has to be able to read
        whether it was.
        """
        try:
                        ...
                        async with _rec_lock:
                            ...
                            if close_data and close_data["close_price"] > 0:
                                ...
                                try:
                                    ...
                                    est_exit = 0
                                if est_exit <= 0:
                                    # UNREAD, not the entry price. Ten retries
                                    # and a dead ticker is an absence; booking
                                    # the exit at the entry turns it into an
                                    # exact break-even nobody measured. The
                                    # position IS gone from the venue, so book
                                    # the close and price none of it.
                                    est_exit = None
                                    reason = "CLOSED (unknown)"
                                    fill_source = f"unpriced_after_{_retries}_retries"
                                else:
                                    reason = self._infer_close_reason(pos, est_exit)
                            ...
                            if est_exit is None:
                                msg += (
                                    ...
                                    f"{_retries} attempts — left out of win rate "
                                    "and realized PnL rather than booked at the "
                                    "entry price as a break-even nobody measured."
                                )
                            self._last_close_data = {
                                "trade_id": pos.trade_id,
                                "symbol": pos.symbol,
