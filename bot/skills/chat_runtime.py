...
_CHAT_NO_TOOLS_RULE = (
    ...
    "you can refer back to them. Each carries its age ('[recorded 2 h ago "
    "— as of then, not now]'): a figure in one is as of that moment, so say "
    "when it is from rather than presenting it as current, and say a fresh "
    "reading is needed when the user asks about now. NEVER write such a "
    "block yourself. You "
...
_CHAT_TOOLS_RULE = (
    ...
    "runtime after a tool really ran. Every earlier turn older than a minute "
    "is prefixed with its age ('[3 d ago]'), and a recorded result with "
    "'[recorded 3 d ago — as of then, not now]': a figure in an aged record "
    "describes THAT moment. Never restate one as the current state — call "
    "the tool again, or say when the figure is from. "
    "NEVER write such a block yourself, and "
