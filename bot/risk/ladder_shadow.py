...
PERSISTENCE. ``data/ladder_ledger.json`` in the shadow book's shape: the
newest ``MAX_ROWS`` kept, atomic writes. A file that will not parse is NOT
...
logger = logging.getLogger("runeclaw.ladder_shadow")

_STATE_DIR = os.environ.get("RUNECLAW_STATE_DIR", "data")
DEFAULT_STATE_FILE = os.path.join(_STATE_DIR, "ladder_ledger.json")

MAX_ROWS = 500
"""The newest rows kept. Stated on the card only when the record is FULL, and
as MAY: exactly MAX_ROWS sized evaluations fill it too, so full does not prove
...
class LadderLedger:
    """The persisted record. ``record`` never raises: the gate must not lose a
    trade to a ledger fault, so a write that fails is logged and answers
    False, and the card reads what is in memory."""
    ...
    def _load(self) -> None:
        try:
            ...
            logger.warning("ladder ledger %s could not be read (%s); it is kept as "
                           "it is and rows recorded from here on stay in memory",
                           self.state_file, self.load_detail)
            return
        rows = data.get("rows") if isinstance(data, dict) else None
        if not isinstance(rows, list) or any(not isinstance(r, dict) for r in rows):
            self.load_state = STATE_UNREADABLE
            self.load_detail = "not a ladder ledger"
            logger.warning("ladder ledger %s is not a ledger; it is kept as it is "
                           "and rows recorded from here on stay in memory",
                           self.state_file)
            return
    ...
    def record(self, row: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning("ladder ledger: a row could not be recorded (%s)",
                           type(exc).__name__)
            return False
    ...
    @property
    def full(self) -> bool:
        return len(self._rows) >= MAX_ROWS
...
def render_ladder_report(ledger: LadderLedger, risk_cfg: Any,
    ...
    if ledger.load_state == STATE_UNREADABLE:
        lines.append(f"⚠️ The record on disk (<code>{html.escape(ledger.state_file)}</code>) "
                     ...
                     f"evaluation(s) recorded in memory since {_stamp(ledger.loaded_at)}.")
        if not rows:
            return "\n".join(lines)
    elif not rows:
        ...
        return "\n".join(lines)
    s = summarize(rows)
    span = f"{_stamp(s.first_ts)} → {_stamp(s.last_ts)}" if s.n else "no readable row"
