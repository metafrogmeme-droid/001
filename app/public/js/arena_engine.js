...
(function (root, factory) {
  // UMD. This file is the ONE implementation of the Arena's rules, and it is
  // in public/js so both halves can load the same bytes: `lib/arena.js`
  // re-exports it for the server, and the browser sandbox loads it directly.
  //
  // The alternative was a second copy for the practice sandbox, and a practice
  // mode that computes liquidation differently from the Arena teaches a habit
  // that costs money the first time it is used for real. There is no version
  // of that which is acceptable, so there is no second copy.
