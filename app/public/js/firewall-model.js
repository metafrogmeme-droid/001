...
(function (root, factory) {
  ...
  const RULES = [
    { kind: 'injection', severity: 'high', id: 'inj_override', title: 'Prompt-injection instruction',
    ...
    { kind: 'injection', severity: 'high', id: 'inj_role', title: 'Role / system-prompt hijack',
    ...
    { kind: 'injection', severity: 'medium', id: 'inj_noconfirm', title: 'Act-without-confirmation lure',
    ...
    { kind: 'secret', severity: 'high', id: 'secret_seed', title: 'Seed-phrase / private-key request',
    ...
    { kind: 'drain', severity: 'high', id: 'drain_sweep', title: 'Drain / sweep language',
    ...
    { kind: 'approval', severity: 'high', id: 'approval_unlimited', title: 'Unlimited approval / setApprovalForAll',
    ...
    { kind: 'urgency', severity: 'low', id: 'urgency', title: 'Urgency / pressure',
    ...
    { kind: 'lure', severity: 'medium', id: 'lure_keywords', title: 'Scam-lure keywords',
    ...
    { kind: 'impersonation', severity: 'medium', id: 'impersonation', title: 'Authority / verification claim',
  ...
  function scanText(input) {
    ...
    for (const r of RULES) {
      ...
      // Carry the stable id through: `kind` is shared by several distinct
      // flags (three injections, four URL checks), so only the id can key a
      // translation without collapsing them into one message.
      if (m) add({ id: r.id, kind: r.kind, severity: r.severity, title: r.title, why: r.why, match: snippet(text, m) });
    }

    // Hidden / obfuscation characters.
    if (ZERO_WIDTH.test(text)) add({ kind: 'hidden', severity: 'high', id: 'hidden_zerowidth', title: 'Zero-width / invisible characters',
      why: 'Invisible characters hide instructions or forge look-alike names that your eyes can’t see but a parser reads.', match: '' });
    if (BIDI.test(text)) add({ kind: 'hidden', severity: 'high', id: 'hidden_bidi', title: 'Right-to-left / bidi override',
    ...
    if (CYRILLIC_GREEK.test(text) && /[a-zA-Z]/.test(text)) add({ kind: 'homoglyph', severity: 'medium', id: 'homoglyph', title: 'Mixed-script (homoglyph) characters',
    ...
    for (const u of urls) {
      ...
      if (/^http:\/\//i.test(u)) add({ kind: 'url', severity: 'medium', id: 'url_http', title: 'Insecure http:// link',
        why: 'A wallet action over plain http can be intercepted. Trusted dapps use https.', match: u.slice(0, 80) });
      if (/xn--/i.test(low)) add({ kind: 'url', severity: 'high', id: 'url_punycode', title: 'Punycode (look-alike) domain',
        why: 'Punycode (xn--) domains impersonate real sites with look-alike Unicode letters.', match: u.slice(0, 80) });
      if (/:\/\/\d{1,3}(\.\d{1,3}){3}(:|\/|$)/.test(low)) add({ kind: 'url', severity: 'high', id: 'url_rawip', title: 'Raw IP-address link',
        why: 'A bare IP instead of a domain is a strong phishing / drainer tell.', match: u.slice(0, 80) });
      if (SHORTENERS.test(low)) add({ kind: 'url', severity: 'medium', id: 'url_shortener', title: 'URL shortener',
    ...
    for (let i = 0; i < uniq.length; i++) {
      for (let j = i + 1; j < uniq.length; j++) {
        ...
        if (a.slice(0, 8) === b.slice(0, 8) && a.slice(-4) === b.slice(-4)) {
          add({ kind: 'address', severity: 'high', id: 'address_poison', title: 'Address-poisoning look-alike',
